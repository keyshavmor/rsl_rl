...
from rsl_rl.ppo_cse.rollout_storage import RolloutStorage
...
class PPO:
    ...
    def __init__(self, actor_critic, device='cpu'):
        ...
        # self.optimizer = optim.Adam(list(self.actor_critic.actor_body.parameters()) + list(self.actor_critic.critic_body.parameters()), lr=PPO_Args.learning_rate)
        # self.optimizer = optim.Adam(list(self.actor_critic.actor_body.parameters()) + \
        #                             list(self.actor_critic.critic_body.parameters()) + \
        #                             list(self.actor_critic.history_encoder.parameters()) , lr=PPO_Args.learning_rate)

        if self.actor_critic.use_adaptation_module:
    ...
    def act(self, obs, privileged_obs, obs_history):
        ...
        self.transition.actions = self.actor_critic.act(obs, obs_history).detach()  # obs_history
    ...
    def update(self):
        ...
        for obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            self.actor_critic.act(obs_batch, obs_history_batch, masks=masks_batch)   # actor input: obs_history + estimated latents
            ...
            if self.actor_critic.use_adaptation_module:
                ...
                for epoch in range(PPO_Args.num_adaptation_module_substeps):
                    ...
                    adaptation_pred = self.actor_critic.adaptation_module(obs_history_batch) # obs_history_batch [2048 * 24 / 4 , 48 * 30] adaptation_pred [2048 * 24 / 4 , 1]
                    with torch.no_grad():
                    ...
                    # print(f"target {adaptation_target[:50, selection_indices].reshape(-1)}")
                    # print(f"estimated {adaptation_pred[:50, selection_indices].reshape(-1)}")
