...
class TDMPC():
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		...
		# Iterate CEM
		for i in range(self.cfg.iterations):  # iterations 6
			actions = torch.clamp(mean.unsqueeze(1) + std.unsqueeze(1) * \
				torch.randn(horizon, self.cfg.num_samples, self.cfg.action_dim, device=std.device), -1, 1)
			if num_pi_trajs > 0:
				actions = torch.cat([actions, pi_actions], dim=1) # actions [1, 537, 12] pi_actions [1, 25, 12]

			# Compute elite actions
			value = self.estimate_value(z, actions, horizon).nan_to_num_(0)  # [537, 1] 
			elite_idxs = torch.topk(value.squeeze(1), self.cfg.num_elites, dim=0).indices # num_elites 64
			elite_value, elite_actions = value[elite_idxs], actions[:, elite_idxs]

			# Update parameters
			max_value = elite_value.max(0)[0]
			score = torch.exp(self.cfg.temperature*(elite_value - max_value)) # [64, 1]
			score /= score.sum(0)
			_mean = torch.sum(score.unsqueeze(0) * elite_actions, dim=1) / (score.sum(0) + 1e-9) # [1, 12]
			_std = torch.sqrt(torch.sum(score.unsqueeze(0) * (elite_actions - _mean.unsqueeze(1)) ** 2, dim=1) / (score.sum(0) + 1e-9))
			_std = _std.clamp_(self.std, 2)
			mean, std = self.cfg.momentum * mean + (1 - self.cfg.momentum) * _mean, _std

		# Outputs
		score = score.squeeze(1).cpu().numpy() # [64,]
		actions = elite_actions[:, np.random.choice(np.arange(score.shape[0]), p=score)] # [1, 12]
		self._prev_mean = mean
		mean, std = actions[0], _std[0] # [12]
		a = mean
		if not eval_mode:
	...
	def update(self, replay_buffer, step):
		...
		# Representation
		z = self.model.h(self.aug(obs)) # [512, 50]
		zs = [z.detach()]
		...
		for t in range(self.cfg.horizon):
			...
			# Predictions
			Q1, Q2 = self.model.Q(z, action[t]) # action[t] [512, 12]
			z, reward_pred = self.model.next(z, action[t]) # latent dynamic and reward,  reward_pred [512, 1]
			with torch.no_grad():
