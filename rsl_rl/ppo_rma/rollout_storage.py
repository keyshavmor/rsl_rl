...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, obs_history_shape, actions_shape, device='cpu'):
        ...
        # Core
        self.observations = torch.zeros(num_transitions_per_env, num_envs, *obs_shape, device=self.device) # [24, 2048, 48]
        # self.privileged_observations = torch.zeros(num_transitions_per_env, num_envs, *privileged_obs_shape, device=self.device)
        if privileged_obs_shape[0] is not None:
            self.privileged_observations = torch.zeros(num_transitions_per_env, num_envs, *privileged_obs_shape, device=self.device)
        else:
            self.privileged_observations = None
        self.observation_histories = torch.zeros(num_transitions_per_env, num_envs, *obs_history_shape, device=self.device) # [24, 2048, 1440]
        self.rewards = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.actions = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device) # [24, 2048, 12]
        self.dones = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device).byte() # [24, 2048, 1]
        ...
        # self.env_bins = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)  # by why
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        batch_size = self.num_envs * self.num_transitions_per_env  # 2048 * 24 = 49152
        mini_batch_size = batch_size // num_mini_batches # 2048 * 24 // 4 = 12288
        indices = torch.randperm(num_mini_batches*mini_batch_size, requires_grad=False, device=self.device) # [49152]

        observations = self.observations.flatten(0, 1) # [24, 2048, 48] => [49152, 48]
        if self.privileged_observations is not None: 
            privileged_obs = self.privileged_observations.flatten(0, 1) # [24, 2048, 1] => [49152, 1]
        else:
            privileged_obs = None
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                batch_idx = indices[start:end] # [12288]

                obs_batch = observations[batch_idx] # [12288, 48]
                critic_observations_batch = critic_observations[batch_idx]
                if privileged_obs is not None: 
                    privileged_obs_batch = privileged_obs[batch_idx]
                else:
                    privileged_obs_batch = None
                obs_history_batch = obs_history[batch_idx] # [12288, 1440]
                actions_batch = actions[batch_idx]
                target_values_batch = values[batch_idx]
                returns_batch = returns[batch_idx]
                old_actions_log_prob_batch = old_actions_log_prob[batch_idx]
                advantages_batch = advantages[batch_idx]
                old_mu_batch = old_mu[batch_idx] # [12288, 12]
